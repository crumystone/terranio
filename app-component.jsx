            ...
            return (
              <button key={p.id} onClick={() => { onSwitch(p.id); setOpen(false); }} style={{
                ...
                <div style={{ textAlign: "left", flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#F97316" }}>⚡ {p.xp} XP</div>
                </div>
...
function GeographyApp() {
  ...
  const startQuiz = (regions) => {
    ...
    setFeedbackMsg(""); setStreakMsg(""); setQuestionsAttempted(0); setScreen("quiz");
  };
  ...
  const handleAnswer = (option) => {
    ...
    if (isCorrect) {
      ...
      const bonus = newStreak >= 5 ? 15 : newStreak >= 3 ? 12 : 10;
      setScore(s => s + 1); setXp(x => x + bonus);
      ...
      setStreakMsg(""); setShakeWrong(true); setTimeout(() => setShakeWrong(false), 600);
    }
  };

  const nextQuestion = () => {
    if (lives <= 0 || currentQ >= questions.length - 1) {
      setQuestionsAttempted(currentQ + 1);
      updateActiveXP(xp, maxStreak);
      setScreen("results");
    } else {
      setCurrentQ(c => c + 1); setSelected(null); setAnswered(false);
      setFeedbackMsg(""); setStreakMsg("");
    }
  };
              ...
              <div key={p.id} style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 13, color: "#94A3B8", marginTop: 4, display: "flex", gap: 16 }}>
                    <span style={{ color: "#F97316" }}>⚡ {p.xp} XP</span>
    ...
    return (
      <div style={styles.app}>
        ...
        <div style={{ ...styles.container, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", textAlign: "center", paddingTop: 40 }}>
          ...
          <p style={{ fontSize: 17, color: "#64748B", marginBottom: 48, fontWeight: 500, maxWidth: 400 }}>Master capitals across the Americas & Europe</p>
          {totalXP > 0 && <div style={{ marginBottom: 32, padding: "10px 24px", borderRadius: 99, background: "rgba(249,115,22,0.15)", border: "1px solid rgba(249,115,22,0.3)", fontSize: 18, fontWeight: 600, color: "#F97316", display: "flex", alignItems: "center", gap: 12 }}>⚡ {totalXP} XP {activeProfile?.quizCount > 0 && <span style={{ fontSize: 14, color: "#94A3B8" }}>· {activeProfile.quizCount} quizzes</span>}</div>}
          <div style={{ display: "flex", gap: 20, width: "100%", maxWidth: 600 }}>
            <button onClick={() => setScreen("explore")} style={{ flex: 1, padding: 32, borderRadius: 28, background: "linear-gradient(160deg, rgba(52,211,153,0.12), rgba(16,185,129,0.06))", border: "2px solid rgba(52,211,153,0.2)", cursor: "pointer", textAlign: "center", transition: "all 0.3s ease", color: "#F1F5F9", fontFamily: "'Fredoka', sans-serif", animation: "slideUp 0.6s ease-out" }}
              ...
              <div style={{ fontSize: 24, fontWeight: 700, marginBottom: 8, fontFamily: "'Lilita One', sans-serif" }}>Explore</div>
              <div style={{ fontSize: 15, color: "#94A3B8", lineHeight: 1.5 }}>Browse states, provinces & countries with their capitals</div>
            </button>
            <button onClick={() => setScreen("regionSelect")} style={{ flex: 1, padding: 32, borderRadius: 28, background: "linear-gradient(160deg, rgba(34,197,94,0.12), rgba(22,163,74,0.06))", border: "2px solid rgba(34,197,94,0.2)", cursor: "pointer", textAlign: "center", transition: "all 0.3s ease", color: "#F1F5F9", fontFamily: "'Fredoka', sans-serif", animation: "slideUp 0.6s 0.15s ease-out both" }}
              ...
              <div style={{ fontSize: 52, marginBottom: 16 }}>🏆</div>
              <div style={{ fontSize: 24, fontWeight: 700, marginBottom: 8, fontFamily: "'Lilita One', sans-serif" }}>Quiz</div>
              <div style={{ fontSize: 15, color: "#94A3B8", lineHeight: 1.5 }}>10 questions, 3 lives — earn XP like a pro!</div>
            </button>
          </div>
        </div>
      </div>
    ...
    return (
      <div style={styles.app}>
        ...
        <div style={{ ...styles.container, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", gap: 24, marginTop: 16, marginBottom: 36, animation: "slideUp 0.6s 0.6s ease-out both" }}>
            <div style={{ textAlign: "center" }}><div style={{ fontSize: 28, fontWeight: 700, color: "#F59E0B" }}>⚡ {xp}</div><div style={{ fontSize: 13, color: "#64748B", marginTop: 2 }}>XP Earned</div></div>
            <div style={{ width: 1, background: "rgba(255,255,255,0.1)" }} />
            <div style={{ textAlign: "center" }}><div style={{ fontSize: 28, fontWeight: 700, color: "#F87171" }}>❤️ {lives}</div><div style={{ fontSize: 13, color: "#64748B", marginTop: 2 }}>Lives Left</div></div>
            <div style={{ width: 1, background: "rgba(255,255,255,0.1)" }} />
            <div style={{ textAlign: "center" }}><div style={{ fontSize: 28, fontWeight: 700, color: "#34D399" }}>🎯 {pct}%</div><div style={{ fontSize: 13, color: "#64748B", marginTop: 2 }}>Accuracy</div></div>
          </div>
