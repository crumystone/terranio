...
const DAILY_CHALLENGE_MULTIPLIER = 1.5; // 1.5x XP for daily challenge

// ─── SUB-REGIONS (for practice mode) ─────────────────────────────────────
const SUB_REGIONS = {
  usa: [
    { id: "all", label: "All States", filter: null },
    { id: "northeast", label: "Northeast", filter: ["Maine","New Hampshire","Vermont","Massachusetts","Rhode Island","Connecticut","New York","New Jersey","Pennsylvania"] },
    { id: "southeast", label: "Southeast", filter: ["Virginia","West Virginia","Kentucky","Tennessee","North Carolina","South Carolina","Georgia","Florida","Alabama","Mississippi","Arkansas","Louisiana"] },
    { id: "midwest", label: "Midwest", filter: ["Ohio","Indiana","Illinois","Michigan","Wisconsin","Minnesota","Iowa","Missouri","North Dakota","South Dakota","Nebraska","Kansas"] },
    { id: "southwest", label: "Southwest", filter: ["Texas","Oklahoma","New Mexico","Arizona"] },
    { id: "west", label: "West", filter: ["Colorado","Wyoming","Montana","Idaho","Utah","Nevada","California","Oregon","Washington","Alaska","Hawaii"] },
  ],
  canada: [
    { id: "all", label: "All Provinces", filter: null },
    { id: "atlantic", label: "Atlantic", filter: ["Nova Scotia","New Brunswick","Prince Edward Island","Newfoundland and Labrador"] },
    { id: "central", label: "Central", filter: ["Ontario","Quebec"] },
    { id: "prairies", label: "Prairies", filter: ["Manitoba","Saskatchewan","Alberta"] },
    { id: "west", label: "West & North", filter: ["British Columbia","Yukon","Northwest Territories","Nunavut"] },
  ],
  europe: [
    { id: "all", label: "All Countries", filter: null },
    { id: "western", label: "Western Europe", filter: ["France","Germany","Netherlands","Belgium","Luxembourg","Austria","Switzerland","Liechtenstein"] },
    { id: "southern", label: "Southern Europe", filter: ["Spain","Portugal","Italy","Greece","Malta","Vatican City","Croatia","Slovenia","Montenegro","Albania","North Macedonia","Bosnia and Herzegovina","Serbia"] },
    { id: "northern", label: "Northern Europe", filter: ["United Kingdom","Ireland","Iceland","Norway","Sweden","Finland","Denmark","Estonia","Latvia","Lithuania"] },
    { id: "eastern", label: "Eastern Europe", filter: ["Poland","Czech Republic","Slovakia","Hungary","Romania","Bulgaria","Ukraine","Belarus","Moldova","Russia"] },
  ],
  mexico: [
    { id: "all", label: "All States", filter: null },
    { id: "north", label: "Northern", filter: ["Baja California","Baja California Sur","Sonora","Chihuahua","Coahuila","Nuevo León","Tamaulipas","Durango","Sinaloa"] },
    { id: "central", label: "Central", filter: ["Aguascalientes","Guanajuato","Querétaro","Hidalgo","Tlaxcala","Puebla","Mexico City","State of Mexico","Morelos","San Luis Potosí","Zacatecas"] },
    { id: "south", label: "Southern", filter: ["Guerrero","Oaxaca","Chiapas","Tabasco","Veracruz","Campeche","Yucatán","Quintana Roo"] },
    { id: "pacific", label: "Pacific", filter: ["Jalisco","Colima","Michoacán","Nayarit"] },
  ],
};
...
function generateDailyQuestions() {
  ...
  const rng = seededRandom("terranio-daily-" + dateStr);

  // Pick a region based on day (cycles through all 4)
  const regionKeys = Object.keys(REGIONS);
  const regionIdx = Math.abs(dateStr.split("-").reduce((a, b) => a + parseInt(b), 0)) % regionKeys.length;
  const regionKey = regionKeys[regionIdx];
  const region = REGIONS[regionKey];
  const label = regionKey === "europe" ? "country" : regionKey === "canada" ? "province" : "state";

  // Build pool using seeded shuffle
  ...
  const states = seededShuffle(region.states).slice(0, 10);
  const paths = SVG_PATHS[regionKey];
  const allStates = region.states.map(([s]) => s);
  const allCapitals = region.states.map(([, c]) => c);

  const questions = states.map(([state, capital], i) => {
    const typeRoll = rng();
    const hasPath = paths && paths[state] && paths[state].d && paths[state].d.length > 20;

    let q;
    if (typeRoll < 0.3 && hasPath) {
      q = { type: "map_tap", question: `Where is ${state}?`, correct: state, region: regionKey, state, options: [] };
    } else if (typeRoll < 0.5 && hasPath) {
      q = { type: "shape_identify", question: `Which ${label} is this?`, correct: state, region: regionKey, state, shapePath: paths[state].d };
    } else if (typeRoll < 0.75) {
      q = { type: "capital", question: `What is the capital of ${state}?`, correct: capital, region: regionKey, state };
    } else {
      q = { type: "state", question: `${capital} is the capital of which ${label}?`, correct: state, region: regionKey, state };
    }

    // Add options for non-map-tap questions
    if (q.type !== "map_tap") {
      const isCapitalAnswer = q.type === "capital" || q.type === "shape_capital";
      const pool = isCapitalAnswer ? allCapitals : allStates;
      const wrong = seededShuffle(pool.filter(a => a !== q.correct)).slice(0, 3);
      q.options = seededShuffle([q.correct, ...wrong]);
    }
    return q;
  });

  return { questions, regionKey, dateStr };
}

function getDailyChallengeStatus(profileId) {
  try {
    const key = "terranio_daily_" + profileId;
    const saved = localStorage.getItem(key);
    if (!saved) return null;
    const parsed = JSON.parse(saved);
    if (parsed.date === getDailyDateStr()) return parsed; // { date, score, xp }
    return null; // expired — new day
  } catch (e) { return null; }
}

function saveDailyChallengeResult(profileId, score, xpEarned) {
  try {
    const key = "terranio_daily_" + profileId;
    localStorage.setItem(key, JSON.stringify({ date: getDailyDateStr(), score, xp: xpEarned }));
  } catch (e) {}
}
...
const SUB_REGIONS = {
  ...
  mexico: {
    "Northern Mexico": ["Baja California","Baja California Sur","Sonora","Chihuahua","Coahuila","Nuevo León","Tamaulipas","Durango","Sinaloa"],
    "Central Mexico": ["Aguascalientes","Guanajuato","Hidalgo","Jalisco","Mexico City","México","Morelos","Puebla","Querétaro","San Luis Potosí","Tlaxcala","Zacatecas"],
    "Southern Mexico": ["Campeche","Chiapas","Colima","Guerrero","Michoacán","Nayarit","Oaxaca","Quintana Roo","Tabasco","Veracruz","Yucatán"],
  },
  europe: {
    "Western Europe": ["France","Germany","Netherlands","Belgium","Luxembourg","Austria","Switzerland"],
    "Southern Europe": ["Spain","Portugal","Italy","Greece","Malta","Vatican City","Andorra","San Marino","Monaco"],
    "Northern Europe": ["United Kingdom","Ireland","Denmark","Sweden","Norway","Finland","Iceland"],
    "Eastern Europe": ["Poland","Czech Republic","Slovakia","Hungary","Romania","Bulgaria","Slovenia","Croatia","Serbia","Bosnia and Herzegovina","Montenegro","North Macedonia","Albania","Kosovo","Moldova","Ukraine","Belarus"],
    "Baltic & Nordic": ["Estonia","Latvia","Lithuania"],
  },
};

function getTutorMastery(profileId, regionKey) {
  try {
    const key = "terranio_mastery_" + profileId + "_" + regionKey;
    const saved = localStorage.getItem(key);
    return saved ? JSON.parse(saved) : {}; // { "Alabama": { correct: 3, wrong: 1 }, ... }
  } catch (e) { return {}; }
}

function saveTutorMastery(profileId, regionKey, mastery) {
  try {
    const key = "terranio_mastery_" + profileId + "_" + regionKey;
...
function getMasteryLevel(correct, wrong) {
  const total = correct + wrong;
  if (total === 0) return { label: "New", color: "#64748B", icon: "⬜" };
  const pct = correct / total;
  if (total >= 3 && pct >= 0.9) return { label: "Mastered", color: "#22C55E", icon: "🟢" };
  if (total >= 2 && pct >= 0.6) return { label: "Learning", color: "#F59E0B", icon: "🟡" };
...
function GeographyApp() {
  ...
  const [exploreView, setExploreView] = useState("map"); // "map" | "list" | "practice"
  const [practiceQ, setPracticeQ] = useState(null);
  const [practiceAnswer, setPracticeAnswer] = useState(null);
  const [practiceRevealed, setPracticeRevealed] = useState(false);
  const [practiceScore, setPracticeScore] = useState({ correct: 0, total: 0 });
  const [practiceSubRegion, setPracticeSubRegion] = useState("all");
  ...
  // Tutor mode
  const [tutorRegion, setTutorRegion] = useState(null);
  const [tutorSubRegion, setTutorSubRegion] = useState(null);
  ...
  const [tutorCorrectCount, setTutorCorrectCount] = useState(0);
  const [tutorTotalCount, setTutorTotalCount] = useState(0);
  const [tutorMastery, setTutorMastery] = useState({});
  // XP accelerators
  ...
  const startQuiz = (regions, dailyQuestions) => {
    const qs = dailyQuestions || generateQuizQuestions(regions, 10);
  ...
  const generatePracticeQuestion = useCallback((regionKey, subRegionId) => {
    const region = REGIONS[regionKey];
    if (!region) return null;
    // Filter states by sub-region
    const subRegions = SUB_REGIONS[regionKey] || [];
    const subRegion = subRegions.find(s => s.id === subRegionId);
    const statePool = subRegion && subRegion.filter
      ? region.states.filter(([s]) => subRegion.filter.includes(s))
      : region.states;
    if (statePool.length === 0) return null;
    const [state, capital] = statePool[Math.floor(Math.random() * statePool.length)];
    const label = regionKey === "europe" ? "country" : regionKey === "canada" ? "province" : "state";
    // Use full region pool for wrong answers so options aren't too obvious
    const allStates = region.states.map(([s]) => s);
    const allCapitals = region.states.map(([, c]) => c);
    const type = Math.random() < 0.5 ? "capital" : "state";
    const correct = type === "capital" ? capital : state;
    const pool = type === "capital" ? allCapitals : allStates;
    const wrong = shuffle(pool.filter(a => a !== correct)).slice(0, 3);
    const options = shuffle([correct, ...wrong]);
    return {
      type, state,
      question: type === "capital" ? `What is the capital of ${state}?` : `${capital} is the capital of which ${label}?`,
      correct, options,
    };
  }, []);

  const nextPracticeQuestion = useCallback((regionKey, subRegionId) => {
    const q = generatePracticeQuestion(regionKey, subRegionId || "all");
    setPracticeQ(q);
  ...
  const nextQuestion = () => {
    if (lives <= 0 || currentQ >= questions.length - 1) {
      ...
      // Daily challenge bonus: 1.5x
      if (isDaily) {
        finalXP = Math.round(finalXP * DAILY_CHALLENGE_MULTIPLIER);
      }
      setXp(finalXP);
      // Check for level-up
      ...
      if (groupCode && activeProfile) {
        const updatedProfile = { ...activeProfile, xp: newXP, quizCount: (activeProfile.quizCount || 0) + 1, bestStreak: Math.max(activeProfile.bestStreak || 0, maxStreak) };
        syncProfile(updatedProfile);
      }
      // Save daily challenge completion
      if (isDaily && activeProfile) {
        saveDailyChallengeResult(activeProfile.id, score, finalXP);
      }
      setScreen("results");
    } else {
    ...
    return (
      <div style={styles.app}>
        ...
        <div style={{ ...styles.container, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", textAlign: "center", paddingTop: 40 }}>
          ...
          {activeProfile && (() => {
            const dailyStatus = getDailyChallengeStatus(activeProfile.id);
            const { regionKey } = generateDailyQuestions();
            const regionData = REGIONS[regionKey];
            const completed = !!dailyStatus;
            return (
              <div style={{
                width: "100%", maxWidth: 480, marginBottom: 24, borderRadius: 22, overflow: "hidden",
                background: completed ? "rgba(255,255,255,0.03)" : "linear-gradient(135deg, rgba(249,115,22,0.12), rgba(234,88,12,0.06))",
                border: `2px solid ${completed ? "rgba(255,255,255,0.06)" : "rgba(249,115,22,0.25)"}`,
                animation: "slideUp 0.5s 0.1s ease-out both",
              }}>
                <button onClick={completed ? undefined : startDailyChallenge} disabled={completed} style={{
                  width: "100%", padding: "20px 24px", cursor: completed ? "default" : "pointer",
                  background: "transparent", border: "none", color: "#F1F5F9",
                  fontFamily: "'Fredoka', sans-serif", textAlign: "left",
                  display: "flex", alignItems: "center", gap: 16, opacity: completed ? 0.6 : 1,
                }}>
                  <div style={{ fontSize: 40, flexShrink: 0 }}>{completed ? "✅" : "🌟"}</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                      <span style={{ fontFamily: "'Lilita One', sans-serif", fontSize: 19, color: completed ? "#94A3B8" : "#F59E0B" }}>Daily Challenge</span>
                      {!completed && <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 99, background: "rgba(249,115,22,0.2)", color: "#F97316", fontWeight: 700 }}>1.5x XP</span>}
                    </div>
                    <div style={{ fontSize: 13, color: "#64748B" }}>
                      {completed
                        ? `Completed! ${dailyStatus.score}/10 · ⚡ ${dailyStatus.xp} XP earned`
                        : `${regionData.emoji} ${regionData.name} · Resets in ${getTimeUntilMidnight()}`
                      }
                    </div>
                  </div>
                  {!completed && <span style={{ fontSize: 20, color: "#F59E0B" }}>→</span>}
                </button>
              </div>
            );
          })()}
    ...
    return (
      <div style={{ ...styles.app, ...(exploreRegion && exploreView === "map" ? { display: "flex", flexDirection: "column", height: "100vh", overflow: "hidden" } : {}) }}>
        ...
        <div style={{ ...styles.header, flexShrink: 0 }}>
          ...
          {exploreRegion ? (
            <div style={{ display: "flex", borderRadius: 14, overflow: "hidden", border: "1.5px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.04)" }}>
              ...
              <button onClick={() => setExploreView("list")} style={{ padding: "9px 16px", border: "none", cursor: "pointer", background: exploreView === "list" ? "rgba(34,197,94,0.45)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s" }}><ListIcon active={exploreView === "list"} /></button>
              <button onClick={() => { setExploreView("practice"); setPracticeSubRegion("all"); nextPracticeQuestion(exploreRegion, "all"); setPracticeScore({ correct: 0, total: 0 }); }} style={{ padding: "9px 14px", border: "none", cursor: "pointer", background: exploreView === "practice" ? "rgba(249,115,22,0.45)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.2s" }}>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={exploreView === "practice" ? "#fff" : "#64748B"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M2 10l10-5 10 5-10 5z" /><path d="M6 12v5c0 0 3 3 6 3s6-3 6-3v-5" /><line x1="22" y1="10" x2="22" y2="16" />
                </svg>
              </button>
            </div>
          ) : <div style={{ width: 80 }} />}
        ...
        <div style={exploreRegion && exploreView === "map" ? { padding: "0 8px 8px", flex: 1, display: "flex", flexDirection: "column" } : styles.container}>
          {!exploreRegion ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 16 }}>
              ...
              {Object.entries(REGIONS).map(([key, region], i) => (
                <div key={key} style={{ display: "flex", gap: 10, animation: `slideUp 0.4s ${i * 0.1}s ease-out both` }}>
                  <button onClick={() => { setExploreRegion(key); setExploreView("map"); setMapSelectedState(null); }} style={{
                    flex: 1, padding: "24px 28px", borderRadius: 22, background: `linear-gradient(135deg, ${region.color}18, ${region.color}08)`,
                    border: `2px solid ${region.color}33`, cursor: "pointer", display: "flex", alignItems: "center", gap: 20,
                    transition: "all 0.3s", color: "#F1F5F9", fontFamily: "'Fredoka', sans-serif",
                  }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = `${region.color}88`; e.currentTarget.style.transform = "translateX(4px)"; }}
                  ...
                  <button onClick={() => { setExploreRegion(key); setExploreView("practice"); nextPracticeQuestion(key); setPracticeScore({ correct: 0, total: 0 }); }} style={{
                    width: 72, borderRadius: 22, background: "rgba(249,115,22,0.1)",
                    border: `2px solid rgba(249,115,22,0.2)`, cursor: "pointer",
                    display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 4,
                    transition: "all 0.3s", color: "#F97316", fontFamily: "'Fredoka', sans-serif",
                  }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = "rgba(249,115,22,0.5)"; e.currentTarget.style.transform = "translateY(-2px)"; }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(249,115,22,0.2)"; e.currentTarget.style.transform = "translateY(0)"; }}>
                    <span style={{ fontSize: 20 }}>📝</span>
                    <span style={{ fontSize: 10, fontWeight: 700 }}>Practice</span>
                  </button>
                </div>
              ))}
            </div>
          ...
          ) : exploreView === "practice" && practiceQ ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "60vh", padding: "24px 16px" }}>

              {/* Sub-region selector */}
              {SUB_REGIONS[exploreRegion] && (
                <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 8, marginBottom: 20, maxWidth: 420 }}>
                  {SUB_REGIONS[exploreRegion].map(sub => (
                    <button key={sub.id} onClick={() => {
                      setPracticeSubRegion(sub.id);
                      setPracticeScore({ correct: 0, total: 0 });
                      nextPracticeQuestion(exploreRegion, sub.id);
                    }} style={{
                      padding: "7px 14px", borderRadius: 99, fontSize: 13, fontWeight: 600,
                      fontFamily: "'Fredoka', sans-serif", cursor: "pointer", transition: "all 0.2s",
                      background: practiceSubRegion === sub.id ? "rgba(249,115,22,0.2)" : "rgba(255,255,255,0.04)",
                      border: `1.5px solid ${practiceSubRegion === sub.id ? "rgba(249,115,22,0.5)" : "rgba(255,255,255,0.08)"}`,
                      color: practiceSubRegion === sub.id ? "#F97316" : "#94A3B8",
                    }}>{sub.label}</button>
                  ))}
                </div>
              )}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 32 }}>
                <span style={{ fontSize: 14, color: "#22C55E", fontWeight: 700 }}>✓ {practiceScore.correct}</span>
                <span style={{ fontSize: 14, color: "#64748B" }}>/</span>
                <span style={{ fontSize: 14, color: "#F1F5F9", fontWeight: 600 }}>{practiceScore.total} questions</span>
                {practiceScore.total > 0 && <span style={{ fontSize: 13, color: "#64748B" }}>({Math.round(practiceScore.correct / practiceScore.total * 100)}%)</span>}
              </div>

              {/* Question */}
              <div style={{ fontSize: 22, fontWeight: 700, textAlign: "center", marginBottom: 28, maxWidth: 400, lineHeight: 1.4, fontFamily: "'Lilita One', sans-serif", color: "#E2E8F0" }}>
                {practiceQ.question}
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12, width: "100%", maxWidth: 380 }}>
                {practiceQ.options.map((opt, i) => {
                  const isCorrect = opt === practiceQ.correct;
                  const isSelected = practiceAnswer === opt;
                  const revealed = practiceRevealed;
                  let bg = "rgba(255,255,255,0.04)";
                  let border = "1.5px solid rgba(255,255,255,0.08)";
                  if (revealed && isCorrect) { bg = "rgba(34,197,94,0.15)"; border = "1.5px solid rgba(34,197,94,0.5)"; }
                  else if (revealed && isSelected && !isCorrect) { bg = "rgba(239,68,68,0.12)"; border = "1.5px solid rgba(239,68,68,0.4)"; }
                  return (
                    <button key={opt} disabled={revealed} onClick={() => {
                      setPracticeAnswer(opt);
                      setPracticeRevealed(true);
                      setPracticeScore(s => ({ correct: s.correct + (isCorrect ? 1 : 0), total: s.total + 1 }));
                    }} style={{
                      padding: "16px 20px", borderRadius: 16, background: bg, border, cursor: revealed ? "default" : "pointer",
                      color: "#F1F5F9", fontSize: 17, fontWeight: 600, fontFamily: "'Fredoka', sans-serif",
                      textAlign: "left", transition: "all 0.2s", animation: `slideUp 0.3s ${i * 0.05}s ease-out both`,
                    }}>
              ...
              {/* Next / feedback */}
              {practiceRevealed && (
                <div style={{ marginTop: 24, textAlign: "center", animation: "slideUp 0.2s ease-out" }}>
                  <div style={{ fontSize: 16, marginBottom: 16, color: practiceAnswer === practiceQ.correct ? "#22C55E" : "#F87171", fontWeight: 700 }}>
                    {practiceAnswer === practiceQ.correct ? "Nice! 🎉" : `The answer is ${practiceQ.correct}`}
                  </div>
                  <button onClick={() => nextPracticeQuestion(exploreRegion, practiceSubRegion)} style={{
                    padding: "14px 40px", borderRadius: 99, background: "linear-gradient(135deg, #22C55E, #16A34A)",
              ...
              <p style={{ marginTop: 32, fontSize: 13, color: "#475569", textAlign: "center" }}>No lives, no timer — just practice! Switch to Map or List anytime.</p>
            </div>
          ) : (
            <>
      ...
      <div style={styles.app}>
        ...
        <div style={{ ...styles.container, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", textAlign: "center" }}>
          ...
          <div style={{ background: "rgba(255,255,255,0.04)", borderRadius: 18, padding: "16px 24px", marginTop: 16, marginBottom: 8, minWidth: 280, animation: "slideUp 0.6s 0.5s ease-out both", border: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            {isDaily && <div style={{ fontSize: 14, color: "#F59E0B", fontWeight: 700, marginTop: 6 }}>🌟 Daily Challenge — 1.5x XP Bonus!</div>}
